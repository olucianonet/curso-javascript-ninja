...
console.log(text.replace(/Manuel Marques de Sousa/g, 'Luciano Cunha'));
...
console.log(text.replace(/O Centauro de Luvas/g, 
    function(total){        
...
var getMonthNumber = function(month){
    ...
    return months[month] ? months[month] : 'Mês inexistente';
};

console.log( '\nMeses representados por números:' );
console.log(`O mês de Março é representado pelo número ` +
    `${getMonthNumber('março')}`);
console.log(`O mês de Setembro é representado pelo número ` +
    `${getMonthNumber('setembro')}`);
console.log(`O mês de Dezembro é representado pelo número ` + 
...
// formato "13 de junho de 1804" para "13/06/1804"

var regexDate = /(\d{1,2}) de (junho|julho) de (\d{4})/g;
...
var replaceDate = function(regex, day, month, year){
    return `${day}/${getMonthNumber(month)}/${year}}`;
};

console.log( '\nReplace de datas:' );
// text = text.replace(regexDate, function(total, group1){
//     group1 = getMonthNumber(group1);
//     total = '/'.concat(group1).concat('/')
//     return total
// });
// console.log(text.replace(regexDate, function(total, group1){
//     group1 = getMonthNumber(group1);
//     total = '/'.concat(group1).concat('/')
//     return total
// }));
