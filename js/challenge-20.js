...
// window.alert(`Bem vindo ${email = email ? email : 'Desconhecido'}`);
...
$button.addEventListener('click', function(event){
    ...
    if (message) {
        ...
        message = window.confirm('Tem certeza que deseja enviar o formulário?')
        if (message){
...
function isValidEmail(email){
    console.log(email);
    var regx = new RegExp('^[\\w+_.]+@\\w+\\.\\w{2,}(?:\\.\\w{2})?$')
